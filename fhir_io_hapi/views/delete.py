...
def delete(request, resource_type, id):
    ...
    Delete on remote HAPI FHIR Server
    ...
    pass_params = build_params(request.DELETE, skip_parm)
    ...
    if DF_EXTRA_INFO:
        od['parameters'] = request.DELETE.urlencode()
        if settings.DEBUG:
